...
function NewSignup() {
    ...
    const onSubmit = async (e) => {
        ...
        await createUserWithEmailAndPassword(auth, email, password, confirmPassword, studentNumber)
          .then((userCredential) => {
            // Signed in
      ...
      <div className="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
        <div className="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
          <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
            ...
            <div>
                <label htmlFor="student-number" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Your Student No.</label>
                <input type="text" name="studentNumber"  onChange={(e) => setStudentNumber(e.target.value)} value={studentNumber} id="student-number" className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Student Number" required />
              </div>
